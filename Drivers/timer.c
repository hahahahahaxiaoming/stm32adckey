...
{
	timer->IsTimeOut = 0;
	timer->TimeOutVal = timems + timeout;
	timer->TickValCache = timems;
}

uint8_t IsTimeOut(TIMER* timer)
{
	timer->TickValCache = timems;
	//if(timer->TickValCache >= timer->TimeOutVal && timer->TickValCache <= timer->TimeOutVal + ADCKEYSCANTIME)
	if(timer->TickValCache >= timer->TimeOutVal)
	{
		timer->IsTimeOut = 1;
